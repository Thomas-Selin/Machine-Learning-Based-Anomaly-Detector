...
def analyse_anomalies(
    ...
    if result['is_anomaly']:
        ...
        explanations.append(Colors.red(f"Anomaly detected at {result['timestamp']} with error score: {result['error_score']:.4f}"))
        ...
        sorted_services_errors = sorted(zip(services, service_errors), key=lambda x: x[1], reverse=True)
        
        for service, error in sorted_services_errors:
            explanations.append(f"  {service}: {error:.4f}")
        
        # Analyze service relationships
        high_error_services = [service for service, error in sorted_services_errors if error > result['threshold']]
        for service in high_error_services:
            ...
            if service in service_relationships:
                ...
                if related_services:
                    explanations.append(f"  Related services: {', '.join(related_services)}")
                    for related_service in related_services:
                        if related_service in high_error_services:
                            ...
                            explanations.append(f"    Related service '{related_service}' also has high error: {service_errors[related_service_idx]:.4f}")
                else:
                    explanations.append(f"  No related services.")
            else:
                explanations.append(f"  No relationship data available for '{service}'.")
        
        # Identify the most important variables for the anomaly
        num_variables_to_show = min(2, len(features))  # Show up to 2 variables
        most_important_variable_indices = np.argsort(variable_errors)[-num_variables_to_show:][::-1]
        most_important_variables = [features[idx] for idx in most_important_variable_indices]
        most_important_values = [variable_errors[idx] for idx in most_important_variable_indices]
        ...
        most_important_services = [services[np.argmax(service_errors)] for _ in most_important_variable_indices]
        ...
        explanations.append(f"Most influential service for this anomaly: {most_influential_service}\n")
    else:
