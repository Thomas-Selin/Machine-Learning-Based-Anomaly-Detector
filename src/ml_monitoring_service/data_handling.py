...
def normalize_feature(df: pd.DataFrame, feature_name: str) -> pd.DataFrame:
    ...
    if pd.isna(min_value) or pd.isna(max_value):
        logger.warning(f"Feature '{feature_name}' contains only NaN values, setting to 0")
        df[feature_name] = 0
    elif min_value == max_value:
        # Feature has constant value - normalize to 0.5 to indicate "middle" value
        logger.debug(f"Feature '{feature_name}' has constant value {min_value}, normalizing to 0.5")
        df[feature_name] = 0.5
    else:
...
def convert_to_model_input(active_set: str, df: pd.DataFrame) -> Tuple[np.ndarray, List[str], List[str]]:
    ...
    for t, timestamp in enumerate(timestamps):
        for service in services:
            service_data = df[(df['timestamp_nanoseconds'] == timestamp) & 
                              (df['service'] == service)]
    ...
    logger.info(f"Converted data shape: {data.shape} (timesteps={len(timestamps)}, services={len(services)}, features={len(features)})")
...
def get_timestamp_of_latest_data(active_set: str) -> Optional[str]:
    """
    ...
    """
    logger.info(f"Checking for existing data timestamps for service set '{active_set}'...")
    ...
    if os.path.exists(training_path):
        try:
            ...
            if 'timestamp' in df.columns:
                ...
                logger.info(f"Found data in {training_path} with latest timestamp: {max_time}")
                logger.info(f"Found data in {training_path} with earliest timestamp: {min_time}")
    ...
    if latest_timestamp:
        ...
        logger.info(f"Most recent data timestamp for '{active_set}': {formatted_timestamp}")
        return formatted_timestamp
    else:
        logger.info(f"No existing training data found for '{active_set}', will download from scratch")
        return None
...
class ServiceMetricsDataset(Dataset):
    ...
    def __init__(self, metrics: np.ndarray, timestamps: pd.Series, window_size: int):
        ...
        self.timestamps = pd.to_datetime(timestamps)  # Ensure timestamps are pandas datetime
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        hours = torch.tensor([ts.hour / 24.0 for ts in timestamp_window]).float().unsqueeze(1)
        minutes = torch.tensor([ts.minute / 60.0 for ts in timestamp_window]).float().unsqueeze(1)
        days = torch.tensor([ts.dayofweek / 7.0 for ts in timestamp_window]).float().unsqueeze(1)
        seconds = torch.tensor([ts.second / 60.0 for ts in timestamp_window]).float().unsqueeze(1)
