...
def load_model(model_filename: str, num_features: int, num_services: int, config: conf.ServiceSetConfig, override_threshold: float = None) -> AnomalyDetector:
    ...
    try:
        ...
        if override_threshold is not None:
            ...
            logger.info(f"Using override threshold: {override_threshold:.6f} (saved threshold was: {checkpoint['threshold']:.6f})")
        else:
        ...
        logger.info(f"Memory usage after loading model '{model_filename}': {get_memory_usage()}")
        return detector
...
def inference(active_set: str, model_filename: str) -> None:
    ...
    with mlflow.start_run(run_name=f"Model inference: {active_set}-microservice-set"):
        ...
        logger.info(Colors.blue(f"⏱️  INFERENCE PROCESS STARTED FOR SERVICE SET: {active_set}. TIME: {start_time.strftime('%Y-%m-%d %H:%M:%S')}"))
        ...
        detector = load_model(model_filename, num_features, num_services, config, override_threshold)
        ...
        if total_possible_windows <= 0:
            logger.warning(f"Not enough data for inference. Data length: {len(data)}, Window size: {detector.window_size}")
            mlflow.log_param("inference_error", "Insufficient data for windowing")
            return
        
        logger.info(f"Processing {total_possible_windows} possible windows with step size {detector.window_size}")
        ...
        for i in range(0, len(data) - detector.window_size, detector.window_size):
            ...
            logger.debug(f"Window {num_windows_processed}: Error score: {result['error_score']:.6f}, Threshold: {result['threshold']:.6f}, Is anomaly: {result['is_anomaly']}")
            
            if result['is_anomaly']:
                logger.warning(f"🚨 ANOMALY DETECTED in window {num_windows_processed} at timestamp {timestamp}")
                logger.warning(f"   Error score: {result['error_score']:.6f} (threshold: {result['threshold']:.6f})")
                explanations = analyse_anomalies(result, config.relationships, services, features)
                logger.info("\n".join(explanations))
                num_anomalies += 1
            
            # Aggregate service errors
            for service, error in zip(services, result['service_errors']):
                service_errors[service] += error

        # Log summary of detection
        if num_anomalies == 0:
            logger.info(f"✅ No anomalies detected in {num_windows_processed} windows processed.")
            logger.info(f"   All error scores were below threshold of {detector.threshold:.6f}")
        else:
            logger.warning(f"⚠️  Detected {num_anomalies} anomalies out of {num_windows_processed} windows processed ({100*num_anomalies/num_windows_processed:.1f}%)")
        ...
        logger.info(Colors.blue(f"✅ INFERENCE PROCESS FINISHED FOR SERVICE SET: {active_set}."))
        ...
        visualize_microservice_graph(config.relationships, result_graph_path, service_errors)
...
if __name__ == '__main__':
    ...
    for active_set in active_sets:
        ...
        logger.info(f"Training Lookback Hours: {service_set_config.training_lookback_hours}")
        logger.info(f"Inference Lookback Minutes: {service_set_config.inference_lookback_minutes}")
        logger.info(f"Window Size: {service_set_config.window_size}")
        logger.info(f"Anomaly Threshold Percentile: {service_set_config.anomaly_threshold_percentile}")
