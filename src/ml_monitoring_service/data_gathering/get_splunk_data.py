...
def start_search(search_query: str) -> Optional[str]:
    """
    ...
    try:
        ...
        try:
            ...
            if not job_id:
                logger.error("No job ID received. Check the search format and authentication.")
...
def get_search_results(job_id: str) -> Optional[List[Dict[str, Any]]]:
    """
    ...
    EXCLUDED_FIELDS = {'info_min_time', 'info_max_time', 'info_search_time', 'search_name', 'search_now'}
    ...
    try:
        ...
        while status_check_count < MAX_JOB_STATUS_CHECKS:
            try:
                ...
                logger.debug(f"Job status: {job_status}, waiting {wait_time}s before next check...")
        ...
        if status_check_count >= MAX_JOB_STATUS_CHECKS:
            logger.error(f"Job {job_id} did not complete after {MAX_JOB_STATUS_CHECKS} status checks")
            return None
...
def delete_search_job(job_id: str) -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.warning(f"Failed to delete job {job_id}. Status: {response.status_code}")
            return False
    except Exception as e:
...
def cleanup_all_search_jobs() -> int:
    ...
    try:
        ...
        if user_response.status_code != 200:
            logger.warning(f"Failed to retrieve current user. Status: {user_response.status_code}")
            return 0
            
        try:
            user_data = user_response.json()
            username = user_data.get("entry", [{}])[0].get("content", {}).get("username")
            ...
            if response.status_code != 200:
                logger.warning(f"Failed to retrieve jobs list. Status: {response.status_code}")
                return 0
            ...
            logger.info(f"Successfully cleaned up {deleted_count}/{len(jobs)} jobs for user {username}")
...
def download_splunk_data_in_batches(
    task: str, 
    active_set: str, 
    timestamp_latest_data: Optional[str] = None
    ...
    if timestamp_latest_data:
        if isinstance(timestamp_latest_data, str):
            try:
                ...
                latest_data_dt = datetime.strptime(timestamp_latest_data, "%m/%d/%Y:%H:%M:%S")
                
                # Calculate how far back we would go with training_lookback_hours
                current_time = datetime.now()
                training_lookback_time = current_time - timedelta(hours=training_lookback_hours)
    ...
    while True:
        logger.info(f"Fetching batch {batch_count}: earliest={earliest_time}, latest={latest_time}")
        ...
        if batch_size < BATCH_LIMIT:
            logger.info(f"Completed: {batch_count} batches, {len(all_results)} total records")
            break
        ...
        try:
            ...
            if not timestamp:
                logger.error("Could not find timestamp in last record to continue batch processing")
                break
    ...
    if DATA_SUBSET:
        try:
            ...
            logger.warning(f"Invalid SUBSET value '{DATA_SUBSET}': {e}. Using all results")
    elif DATA_SAMPLING:
        try:
            import random
            ...
            if 0 < sampling_rate < 1:
                ...
                logger.warning(f"SAMPLING must be between 0 and 1. Got {sampling_rate}. Using all results")
        except (ValueError, TypeError) as e:
            logger.warning(f"Invalid SAMPLING value '{DATA_SAMPLING}': {e}. Using all results")
