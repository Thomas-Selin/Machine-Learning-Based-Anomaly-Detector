...
def query_prometheus(
    ...
    while retries < MAX_RETRIES:
        try:
            ...
            logger.warning(f"Error querying Prometheus (attempt {retries + 1}/{MAX_RETRIES}): {e}")
            retries += 1
            if retries < MAX_RETRIES:
                sleep_time = sleep_interval * (RETRY_BACKOFF_FACTOR ** retries)  # Exponential backoff
...
def process_results(results: List[Dict[str, Any]], data_type: str) -> List[Dict[str, Any]]:
    """
...
def download_prometheus_data(
    ...
    try:
        ...
        if all_processed_data:
            ...
            directory = Path("output") / active_set / f"prometheus_data_{task}" / service_name_cleaned
            ...
            logger.warning(f"No results obtained from Prometheus for {service_name} ({data_type})")
    finally:
...
def get_timestamps_for_service(splunk_data: List[Dict[str, Any]], service_name: str) -> List[str]:
    """
    ...
    """
    timestamps = {entry['timestamp'] for entry in splunk_data if entry.get('service') == service_name}
...
def main(task: str, active_set: str) -> None:
    ...
    for service_name in services:
        ...
        for metric_type in conf.get_metrics(active_set):
            download_prometheus_data(task, service_name, metric_type, timestamps, active_set)
