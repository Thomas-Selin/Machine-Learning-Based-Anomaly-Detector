...
def combine_services(task: str, active_set: str, age_latest_data: str | None) -> None:
    ...
    for metric in conf.get_metrics(active_set):
        for service in active_services:
            ...
            service_file_path = (
                base_output_path / service_clean / f"{metric}-{service_clean}.json"
            )
    ...
    if not json_files:
        logger.warning(
            f"No JSON files found for task '{task}' and active_set '{active_set}'"
        )
        return
    ...
    for json_file in json_files:
        try:
            ...
            df["timestamp"] = pd.to_datetime(df["timestamp"], errors="coerce").dt.round(
                "ms"
            )
    ...
    if not dataframes:
        logger.error(
            f"No valid dataframes to concatenate for task '{task}' and active_set '{active_set}'"
        )
        raise ValueError("No dataframes to concatenate")

    combined_prometheus_df = pd.concat(
        dataframes, axis=0, ignore_index=True, copy=False
    )

    # Group by stable time key + service and aggregate using 'first' to combine metrics into a single row
    combined_prometheus_df = combined_prometheus_df.groupby(
        ["timestamp_nanoseconds", "service"], as_index=False
    ).first()
    ...
    new_dataset = pd.merge(
        combined_prometheus_df,
        splunk_data,
        on=["timestamp_nanoseconds", "service"],
        how="outer",
    )

    # Ensure human-readable timestamp exists (from key)
    if "timestamp" not in new_dataset.columns or new_dataset["timestamp"].isna().any():
        new_dataset["timestamp"] = pd.to_datetime(
            new_dataset["timestamp_nanoseconds"], unit="ns"
        )
        ...
        try:
            ...
            if not pd.api.types.is_datetime64_any_dtype(existing_dataset["timestamp"]):
                existing_dataset["timestamp"] = pd.to_datetime(
                    existing_dataset["timestamp"], errors="coerce"
                )
                existing_dataset = existing_dataset.dropna(subset=["timestamp"])

            # Concatenate existing data with new data
            dataset = pd.concat(
                [existing_dataset, new_dataset], axis=0, ignore_index=True, copy=False
            )

            # Remove duplicates based on timestamp and service
            dataset = dataset.drop_duplicates(
                subset=["timestamp", "service"], keep="last"
            )
            ...
            if task == "training" and age_latest_data is not None:
                ...
                cutoff_time = current_time - timedelta(
                    hours=conf.get_training_lookback_hours(active_set)
                )

                # Filter out data older than the cutoff time
                dataset = dataset[dataset["timestamp"] >= cutoff_time]
                logger.info(
                    f"Filtered training data to {len(dataset)} rows (cutoff time: {cutoff_time})"
                )
        except Exception as e:
            logger.error(
                f"Error processing existing dataset from {output_file_path}: {e}"
            )
    ...
    try:
        dataset.to_json(
            output_file_path, orient="records", date_format="iso", date_unit="us"
        )
