...
class HybridAutoencoderTransformerModel(nn.Module):
    ...
    def __init__(self, num_services, num_features):
        super(HybridAutoencoderTransformerModel, self).__init__()
        self.hidden_dim = 64
        ...
        self.service_embeddings = nn.Parameter(torch.randn(num_services, self.hidden_dim))
    ...
    def forward(self, x, time_features):
        ...
        for service_idx in range(num_services):
            ...
            service_input = torch.cat([service_data, time_features], dim=2)  # [batch, seq, features+4]
        ...
        stacked_reshaped = stacked.reshape(batch_size * seq_len, num_services, self.hidden_dim)
        attn_out, _ = self.cross_service_attention(stacked_reshaped, stacked_reshaped, stacked_reshaped)
        ...
        transformer_input = stacked.contiguous().view(batch_size, seq_len, -1)  # [batch, seq, services*hidden]
        ...
        # Reshape and decode
        transformer_out = transformer_out.view(batch_size, seq_len, num_services, self.hidden_dim)
