...
6. To run tests, start MLflow by running `poetry run mlflow server`, then run the terminal command `poetry run pytest` from the repo root folder. To run end-to-end tests, run `poetry run pytest -m integration`
...
By default, inference uses the anomaly threshold persisted from training/validation.

This is off-by-default for production safety:

- **Contamination risk:** If you recompute the threshold on the same inference window you are trying to detect anomalies in, true anomalies can raise the threshold and become harder to detect.
- **Stability:** A persisted threshold makes anomaly rates more comparable over time and less sensitive to short-term variance.

If you explicitly want to recompute the threshold from the current inference dataset (useful for experiments or strong drift scenarios), set `RECALCULATE_THRESHOLD_ON_INFERENCE=true`.
Use with care and prefer recalculating on a trusted “baseline” window (known-good traffic) rather than during an incident.
