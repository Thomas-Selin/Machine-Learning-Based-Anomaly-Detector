...
from ml_monitoring_service.configuration import ConfigLoader, get_services, get_metrics
...
def test_process_results():
    ...
    assert processed_data == [{'timestamp': datetime.datetime(2021, 5, 31, 16, 13, 45), 'cpu': 1.0}]
...
def test_download_prometheus_data(monkeypatch, config):
    ...
    monkeypatch.setattr('ml_monitoring_service.data_gathering.get_prometheus_data.query_prometheus', mock_query_prometheus)
    
    # Create test service directory
    service_name = "test-service"
    service_name_underscore = service_name.replace('-', '_') #TODO standardize the name case style
    active_set = "test-microservice-set"
    service_dir = Path(f"output/{active_set}/prometheus_data_training/{service_name_underscore}")
    service_dir.mkdir(parents=True, exist_ok=True)
    
    # Test download
    download_prometheus_data("training", service_name, 'cpu', ['2022-01-01 00:00:00.000'], active_set)
...
def test_main_with_service_set(monkeypatch, config):
    ...
    # Mock functions
    monkeypatch.setattr('ml_monitoring_service.data_gathering.get_prometheus_data.read_splunk_data', mock_read_splunk_data)
    monkeypatch.setattr('ml_monitoring_service.data_gathering.get_prometheus_data.download_prometheus_data', mock_download_prometheus_data)
...
def test_convert_to_timestamp():
    ...
    from ml_monitoring_service.data_gathering.get_prometheus_data import convert_to_timestamp
...
def test_convert_to_timestamp_with_milliseconds():
    ...
    from ml_monitoring_service.data_gathering.get_prometheus_data import convert_to_timestamp_with_milliseconds
    dt = datetime.datetime(2022, 1, 1, 12, 0, 0)
...
def test_get_timestamps_for_service():
    ...
    from ml_monitoring_service.data_gathering.get_prometheus_data import get_timestamps_for_service
    splunk_data = [
        {'service': 'service1', 'timestamp': '2022-01-01 00:00:00.000'},
